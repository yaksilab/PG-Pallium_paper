%% Plotting the fraction of neurons exhibiting additivity or depression (Fig. 6E and 6F)
%This script plots the fraction of neurons classified using their response characteristics using their interaction index (this measure quantify the amount of additivity of the neuron's response).
%The indices of the classified Pallial neurons were obtained using the script: InVivo_PalNeurons_LightTapBoth_Additivity.m
...
load(analysis_path);
%% CDF plot or histogram of multisensory indices (Fig. 6E)
gray=[0.65 0.65 0.65];
interaction_idx=[];

for fish={'fish1','fish2','fish3','fish4','fish5','fish6','fish7','fish8','fish9','fish10'} 
    temp=Analysis_PalliumMultiModal_Filt.(fish{1}).Pallium.additivity.interaction_idx;
    interaction_idx=[interaction_idx;temp];
end

f2=figure();
f2.Units="centimeters";
f2.Position=[10 15 6 8];
h=cdfplot(interaction_idx);
h.LineWidth=1.5;
h.Color='k';
% histogram(interaction_idx,'BinEdges',binEdges);
hold on
grid off;
box off;
...
xticks([-300:100:300]);
xline(0,'LineStyle','-.','Color',gray,'linewidth', 1.5);
xline(100,'LineStyle','-','Color',gray,'linewidth', 0.75);
title('Multisensory interaction index');
...
%% Plotting the fraction of neurons exhibiting additivity or depression (Fig. 6F)
%Defining the general variables
Perc_supperAdd_Pal=[];
Perc_depressed_Pal=[];
Perc_subAdd_Pal=[];
...
for fish={'fish1','fish2','fish3','fish4','fish5','fish6','fish7','fish8','fish9','fish10'}
    ...
    pos_Pal=PalNeurons.(fish{1}).positions_norm_Pal;
    ...
    %Calculating the percentage of enhanced/suppressed cells in the pallium
    perc_super=numel(idx_superAdd)/size(pos_Pal,1);
    perc_depressed=numel(idx_depressed)/size(pos_Pal,1);
    perc_sub=numel(idx_subAdd)/size(pos_Pal,1);

    %Regrouping the data
    Perc_supperAdd_Pal=[Perc_supperAdd_Pal;perc_super];
    Perc_depressed_Pal=[Perc_depressed_Pal;perc_depressed];
    Perc_subAdd_Pal=[Perc_subAdd_Pal;perc_sub];
end

%Calcuating the Mean
m_Perc_supperAdd_Pal=mean(Perc_supperAdd_Pal,1,"omitnan");
...
sem_Perc_supperAdd_Pal=std(Perc_supperAdd_Pal,"omitnan")/sqrt(length(Perc_supperAdd_Pal));
...
data=[m_Perc_depressed_Pal;...
    m_Perc_subAdd_Pal;...
    m_Perc_supperAdd_Pal];
data=data*100;
sem=[sem_Perc_depressed_Pal;...
    sem_Perc_subAdd_Pal;...
    sem_Perc_supperAdd_Pal];
sem=sem*100;
