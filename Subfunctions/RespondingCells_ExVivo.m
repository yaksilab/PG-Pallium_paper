%% Classifying the Pallial neurons following a PG stimulation ex-vivo (Subfunction)
%This script classifies whether a neuron is "responding" to the PG micro-stimulation or not.
...
%%
function [positive_resp_cells,negative_resp_cells]=RespondingCells_ExVivo(traces,frame_rate,n_stim,resp_time)
...
for cell=1:size(m_traces_test,2) %loop over the # of neurons
    ...
    elseif abs(post2) > abs(pre)+(2.5*pre_std)
        if post2 < pre 
           neg_list(cell) = 1; 
       end
    end
