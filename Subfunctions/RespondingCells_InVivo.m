%% Classifying the neurons (or pixels) following the delivery of a sensory stimulation in vivo (Subfunction)
...
%%
function [positive_resp_cells,negative_resp_cells]=RespondingCells_InVivo(traces,frame_rate,n_stim,resp_time)
...
for cell=1:size(m_traces_test,2) %loop over the # of neurons
    ...
    elseif abs(post) > abs(pre)+(1.5*pre_std)
        if post < pre
           neg_list(cell) = 1; 
       end
    end
