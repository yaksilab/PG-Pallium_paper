%% Example neural traces of the Pallial neurons during sensory stimulation (Fig. 5E)
%This script plots the mean neural responses for the example pallial neurons during light and vibration stimulation.
...
close all;
f=1;
for fish={'fish1','fish3','fish4'} %Can change the fish number here to visualize data from other fish

%Defining the parameters
frame_rate=PalNeurons.(fish{1}).volume_rate;
cond=PalNeurons.(fish{1}).stim_cond;

idx_lightExcl=Analysis_PalliumMultiModal_Filt.(fish{1}).Pallium.idx_pos_lightExcl;
idx_vibExcl=Analysis_PalliumMultiModal_Filt.(fish{1}).Pallium.idx_pos_tapExcl;
idx_multiSens=Analysis_PalliumMultiModal_Filt.(fish{1}).Pallium.idx_pos_multiSens;
...
if f==1
    ...
    eg_idx_vibExcl=idx_vibExcl(54);
    eg_idx_multiSens=idx_multiSens(30);
end

if f==2
    eg_idx_lightExcl=idx_lightExcl(23);
    % eg_idx_vibExcl=idx_vibExcl(24);
    % eg_idx_multiSens=idx_multiSens(8);
    eg_idx_vibExcl=NaN;
    eg_idx_multiSens=NaN;
end

if f==3
    eg_idx_lightExcl=idx_lightExcl(8);
    eg_idx_vibExcl=idx_vibExcl(18);
    eg_idx_multiSens=idx_multiSens(20);
end
...
data=PalNeurons.(fish{1}).TrialDFF_filt;
data_Pal=data(resp_on:resp_off,:,:);
    ...
    for condition=1:3 %for either light excl, vib excl, multi-sensory
        ...
        %Defining the neural data
        data_light=data_Pal(:,cond(:,1),resp_select)';
        data_vib=data_Pal(:,cond(:,2),resp_select)';
