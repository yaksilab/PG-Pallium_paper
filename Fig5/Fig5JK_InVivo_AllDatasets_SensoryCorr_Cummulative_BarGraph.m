%% Plots the sensory correlations of multi-sensory units across all datasets (Fig. 5J, 5K)
%This scripts plots the correlation between the sensory responses for the multi-sensory units (neurons and axonal pixels) 
% across all datasets as a cummulative plot (5J) and then as a bar plot (5K).
...
%% Correlations
Corr_LT_m_Resp_all_PG={};
m_Corr_PG=[];

for i=1:11 % number of fish
    m_LightResp=sensory_respPG{i,1}';
    m_TapResp=sensory_respPG{i,2}';
    
    %for same cells only
    cross_corr_matrix = nan([size(m_LightResp,1), size(m_LightResp,1)]); % estabilishing matrix
    cross_corr=[];
    for cell = 1:size(m_LightResp,1)
           R = corrcoef(m_LightResp(cell,:),m_TapResp(cell,:)); 
           temp=R(1,2);
           cross_corr=[cross_corr;temp];
           cross_corr_matrix(cell, cell) = R(1,2);
    end
    m_temp=nanmean(cross_corr);
    Corr_LT_m_Resp_all_PG{i,1}=cross_corr;
    m_Corr_PG=[m_Corr_PG;m_temp];

end

Corr_PG=[];
for k=1:11
    temp=Corr_LT_m_Resp_all_PG{k,1};
    Corr_PG=[Corr_PG;temp];
...
%% Correlations
Corr_LT_m_Resp_all_PGAxon={};
m_Corr_PGAxon=[];

for i=1:12 %number of fish
    ...
    m_temp=nanmean(cross_corr);
    Corr_LT_m_Resp_all_PGAxon{i,1}=cross_corr;
    m_Corr_PGAxon=[m_Corr_PGAxon;m_temp];

end

Corr_PGAxons=[];
for k=1:12
    temp=Corr_LT_m_Resp_all_PGAxon{k,1};
    Corr_PGAxons=[Corr_PGAxons;temp];
...
%%
Corr_PalNeurons_m_resp_all_pallium={};
m_Corr_Pallium=[];

for i=1:10 %number of fish
    m_Lightresp_pal=sensory_resp_pal{i,1}';
    m_Tapresp_pal=sensory_resp_pal{i,2}';
    
    cross_corr_matrix = nan([size(m_Lightresp_pal,1), size(m_Lightresp_pal,1)]); % estabilishing matrix
    cross_corr=[];
    for cell = 1:size(m_Lightresp_pal,1)
           R = corrcoef(m_Lightresp_pal(cell,:),m_Tapresp_pal(cell,:)); 
           temp=R(1,2);
           cross_corr=[cross_corr;temp];
           cross_corr_matrix(cell, cell) = R(1,2);
    end
    m_temp=nanmean(cross_corr);
    Corr_PalNeurons_m_resp_all_pallium{i,1}=cross_corr;
    m_Corr_Pallium=[m_Corr_Pallium;m_temp];

end

Corr_Pallium=[];
for k=1:10
    temp=Corr_PalNeurons_m_resp_all_pallium{k,1};
    Corr_Pallium=[Corr_Pallium;temp];

end
%% Plotting the cumulative (correlation) (Fig. 5J)
%Color definition
...
f1.Position=[10 15 7 8];

%plotting the Pallial neurons correlations
c1=cdfplot(Corr_Pallium);
c1.Color=color(3,:);
c1.LineWidth=1.5;

hold on
%plotting the PG axons correlations
c2=cdfplot(Corr_PGAxons);
c2.Color=color(2,:);
c2.LineWidth=1.5;

%plotting the PG neurons correlations
c3=cdfplot(Corr_PG);
c3.Color=color(1,:);
c3.LineWidth=1.5;
...
xlabel('Sensory response correlations');
ylabel('Cummulative fraction');
legend('Pallial neurons','PG axons','PG neurons');
title('All pairwise correlation values (per cell)')
grid off;
...
%% Plotting the combined bargraph (Pallium vs PGaxons vs PG) (Fig. 5K)
% Regrouping the data (correlation)

m_Corr_Pallium2=m_Corr_Pallium;
m_Corr_Pallium2(11:12,:)=NaN;
m_Corr_PG2=m_Corr_PG;
m_Corr_PG2(12,:)=NaN;
all_data=[m_Corr_PG2,m_Corr_PGAxon,m_Corr_Pallium2];

mean_avgCorr_PG=mean(all_data(:,1),"omitnan");
mean_avgCorr_PGaxon=mean(all_data(:,2),"omitnan");
mean_avgCorr_Pallium=mean(all_data(:,3),"omitnan");

sem_avgCorr_PG=std(all_data(:,1),"omitnan")/sqrt(length(all_data(:,1)));
sem_avgCorr_PGaxon=std(all_data(:,2),"omitnan")/sqrt(length(all_data(:,2)));
sem_avgCorr_Pallium=std(all_data(:,3),"omitnan")/sqrt(length(all_data(:,3)));

% PLotting the bar graph (correlation)
data=[mean_avgCorr_PG;mean_avgCorr_PGaxon;mean_avgCorr_Pallium];
sem=[sem_avgCorr_PG;sem_avgCorr_PGaxon;sem_avgCorr_Pallium];
