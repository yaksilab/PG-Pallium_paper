%% K-means Analysis and Calculating the cluster fidelity index (For PG neurons) (Function for Fig. 2)
%This script is based on the one used in Bartoszek et al., (2021): https://doi.org/10.1016/j.cub.2021.08.021 
%This script seperates the PG neuronal activity into different K-means cluters.
%This script examines the percentage of neurons remaining in the same K-means cluster during 2 pre-defined time periods.
...
% perc_clust_fidel_spont_shuffle = percentage of neurons remaining the same cluster following 2 spontaneous activity periods (following a shuffling of the cluster identities)
...
% perc_clust_fidel_light_shuffle = percentage of neurons remaining the same cluster following light stimulation using the centroid obtained from the spontaneous activity period (following a shuffling of the cluster identities)
...
function [perc_clust_fidel_spont,perc_clust_fidel_spont_shuffle,perc_clust_fidel_light,perc_clust_fidel_light_shuffle,...
    perc_clust_fidel_tap,perc_clust_fidel_tap_shuffle,KmeansIdx_spontAll,KmeansIdx_spont1,KmeansIdx_spont2,KmeansIdx_light,KmeansIdx_tap]=Fig2_InVivo_PGNeurons_Kmeans_ClusterFidelity(d_path,s_path);
%% Loading the data
...
for choice =1:2
    ...
    traces_PG_stim=PGNeurons.(fish{1}).TrialDFF_filt;
    ...
    if choice==2
        ...
        light_traces=traces_PG_stim(1:resp_off,cond(:,1),:);
        m_light_traces=squeeze(mean(light_traces,2))';
        
        tap_traces=traces_PG_stim(1:resp_off,cond(:,2),:);
        m_tap_traces=squeeze(mean(tap_traces,2))';
    end
    
    if choice==1
    %% Kmeans for spontaneous data period 1
    data_spont=traces_PG(:,spont_onset:spont_offset);
    ...
    data_spont2=traces_PG(:,spont_onset2:spont_offset2);
    ...
    %% Comparing clusters (calculating the cluster fidelity)
    %Function adapted from Bartoszek et al., (2021)
...
%Note replace the location where you want to save the variables.
 fileName_save='202605_PGNeurons_AnalysisKmeans'; %Change the name of the file here
