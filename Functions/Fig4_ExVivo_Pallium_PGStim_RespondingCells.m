%% Determining the responding and reliable pallial neurons following micro-electrode stimulations in the PG (Function for Fig. 4)
...
%Outputs: 
% Analysis_PG_Stim_Filt: Structure containing the indices for light and vibration responding cells and the single-cell across trials correlations.
...
function [Analysis_PG_Stim_Filt]=Fig4_ExVivo_Pallium_PGStim_RespondingCells(d_path,s_path);
%% Loading the data
...
for fish={'fish1','fish2','fish3','fish4','fish5','fish6','fish7','fish8','fish9','fish10','fish11','fish12'}
    ...
    frame_rate=Pallium_PGStim.(fish{1}).volume_rate;
    stim_onset=ceil(5*frame_rate); 
    
    %Defining the neural activity traces
    org_traces=Pallium_PGStim.(fish{1}).TrialDFF_filt;
    traces=org_traces;

    %% Finding the responding cells
    n_stim=size(traces,2); %number of stimulus presentation
    response_time=5; %baseline duration
    [positive_resp_cells,negative_resp_cells]=RespondingCells_ExVivo(traces,frame_rate,n_stim,response_time); %uses 5s baseline, 5 seconds after stim
    ...
    Analysis_PG_Stim_Filt.(fish{1}).all.all_cell_corr_pVal=all_cell_pVal;
    Analysis_PG_Stim_Filt.(fish{1}).all.all_cell_avg_corr=all_cell_avg_corr;
    Analysis_PG_Stim_Filt.(fish{1}).all.positive_resp_cells=positive_resp_cells;
    Analysis_PG_Stim_Filt.(fish{1}).all.negative_resp_cells=negative_resp_cells;
end

%% Saving
%Note replace the location where you want to save the variables.
 fileName_save='202605_PalNeurons_PGStim_Analysis'; %Change the name of the file here
 save_path=fullfile(s_path,fileName_save);
 save(save_path,'Analysis_PG_Stim_Filt');
end